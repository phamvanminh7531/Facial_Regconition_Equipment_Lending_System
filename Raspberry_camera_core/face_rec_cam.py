...
borrow_request_url = api_url+'student-borrow/'
return_item_request_url = api_url+'item-return/'
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            while True:
                ...
                val, pts, st_code = QRdet.detectAndDecode(frame)

                if val != '':
                    x = requests.get(return_item_request_url+str(val))
                    print(x.text)
                ...
                try:
                    ...
                    if faces_found > 0:
                        ...
                        for i in range(faces_found):
                            ...
                            if best_class_probabilities > DETECT_THRESHOLD:
                                mylcd.lcd_clear()
                                mylcd.lcd_display_string("Face Detected", 1)
                                mylcd.lcd_display_string(best_name, 2)
                                if button.is_pressed:
                                    print("Name: {}, Probability: {}".format(best_name, best_class_probabilities))
                                    x = requests.get(borrow_request_url+str(best_name))
                                    result = x.json()["result"]
                                    print(result)
                                    if result == "Authoreize":
                                        mylcd.lcd_clear()
